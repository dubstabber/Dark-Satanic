...
func test_weeper_left_at_nest_height_glides_down_to_its_flight_height() -> void:
	...
	assert_gt(lowest, 1.15, "bobs around head height, never touching down (lowest %.2f)" % lowest)
	assert_lt(highest, 1.9, "and does not climb away either (highest %.2f)" % highest)
